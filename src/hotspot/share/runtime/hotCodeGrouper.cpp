...
static void wait_for_new_c2_nmethods() {
  ...
  log_info(hotcodegrouper)(
      "Waiting for new C2 nmethods for profiling...");
  MonitorLocker ml(HotCodeGrouper_lock, Mutex::_no_safepoint_check_flag);
  ml.wait(0);  // Wait without timeout until notified
  log_info(hotcodegrouper)(
      "New C2 nmethods detected, starting profiling...");
}
...
static inline int min_samples() {
  ...
  //
  // 15k samples might take a while to collect, we choose to use 8k samples as a compromise.
  // This gives a margin of error of about 0.07% at a confidence level of 95
  return 8000; // TODO: Make this configurable.
}

static inline double min_frequency() {
  // We consider nmethods which appear in at least 0.1% of the samples.
  return 0.001; // TODO: Do we need to make this configurable?
}
...
class HotCodeHeapCandidates : public StackObj {
 ...
 public:
  void find_hot(const NMethodSamples& samples, int total_samples) {
    ...
    if (UseNewCode) {
      _hot_candidates.sort(HotCodeGrouper::nmethodPairCompare);
    }
  }

  void find_cold() {
    auto func = [&](nmethod* nm, uint64_t count) {
      if (count >= HotCodeMaxNotSeenInProfiles) {
        _cold_nmethods.add(nm);
      }

      HotCodeGrouper::_hot_nmethods.put(nm, count + 1);
    };
    HotCodeGrouper::_hot_nmethods.iterate_all(func);
  }
  ...
  void remove_cold() {
    ...
    while (!it.is_empty()) {
      ...
      if (PrintCodeCache) {
        log_trace(hotcodegrouper)("\tRemoving cold nm: <%p> method: <%s>", nm, nm->method()->external_name());
      }

      CompiledICLocker ic_locker(nm);
      if (nm->relocate(CodeBlobType::MethodNonProfiled) != nullptr) {
        HotCodeGrouper::_hot_nmethods.remove(nm);
      }
    }
  }
...
void HotCodeGrouper::group_nmethods(ThreadSampler& sampler) {
  ...
  log_info(hotcodegrouper)("Profiling results: %d samples, %d nmethods, "
      "%d unregistered nmethods",
      total_samples, sampler.samples().number_of_entries(),
      (int)_unregistered_nmethods.size());
